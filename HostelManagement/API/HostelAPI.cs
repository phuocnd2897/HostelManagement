...
namespace HostelManagement.API
{
    public class HostelAPI
    {
        ...
        public List<HostelResponseModel> GetAllHostel()
        {
            try
            {
                var result = BaseAPI.GetMulti<HostelResponseModel>("Hostel", "GetAll", "", _token);
                if (result != null)
                {
        ...
        public HostelRequestModel AddHostel(MultipartFormDataContent hostelRequestModel)
        {
            try
            {
                var result = BaseAPI.PostWithFile<HostelRequestModel>("Hostel", "", hostelRequestModel, _token);
                if (result != null)
                {
                    return result.Result;
                }
        ...
        public HostelRequestModel UpdateHostel(MultipartFormDataContent hostelRequestModel)
        {
            try
            {
                var result = BaseAPI.PutWithFile<HostelRequestModel>("Hostel", "", hostelRequestModel, _token);
                if (result != null)
                {
                    return result.Result;
                }
